...
-- SOURCE: https://pastebin.com/raw/gaz9qf0D
...
saveGuiBtn.TextColor3 = Color3.fromRGB(0,0,0)
...
RunService.RenderStepped:Connect(function(dt)
    if flying and floatPart then
        if character and character:FindFirstChild("HumanoidRootPart") then
            ...
            local pos = floatPart.Position
            local target = Vector3.new(
                hrp.Position.X,
                hrp.Position.Y - hrp.Size.Y/2 - floatPart.Size.Y/2,
                hrp.Position.Z
            )
...
walkBtn.TextColor3 = Color3.fromRGB(0,0,0)
...
local function walkToPosition(targetPos)
    if not targetPos or not humanoid or humanoid.Health <= 0 then return end
...
floatV2Btn.TextColor3 = Color3.fromRGB(255,255,255)
floatV2Btn.Text = "Float V2: OFF"
floatV2Btn.Parent = mainFrame

local flyingV2 = false
...
RunService.RenderStepped:Connect(function(dt)
    if flyingV2 and character and humanoid.Health > 0 then
        local cam = workspace.CurrentCamera
        local lookDir = cam.CFrame.LookVector
        hrp.Velocity = lookDir * flySpeed + Vector3.new(0, -2, 0) -- falling effect
    end
end)
